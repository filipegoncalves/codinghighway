...
void matching_statistics(struct suffix_tree *tree, char *text, size_t text_sz)
{
	size_t text_i = 0;
	size_t curr_start = 0;
	struct st_point curr_point = { tree->root, tree->root, 0 };
	...
	while (curr_start < text_sz-1) {
		assert(text_i < text_sz);
		next_node = can_descend(tree, &curr_point, text[text_i]);
		while (next_node == NULL) {
			printf("%d ", curr_point.string_depth);
			suffix_jump(tree, &curr_point);
			curr_start++;
			next_node = can_descend(tree, &curr_point,
						text[text_i]);
		...
		descend(tree, &curr_point, text[text_i]);
		text_i++;
	}
}
...
int main(void)
{
	...
	while ((op = getchar()) != 'E') {
		...
		matching_statistics(stree, text_buff, text_sz+1);
		printf("\n");
		getchar();
	}
